...
public class DotTreeGUI extends DrawingGUI {
	...
	public void handleMousePress(int x, int y) {
		if (mode == 'a') {
			...
			// if tree is empty, set tree to new quadtree with the dot as one of the parameters,
			// else insert newPoint into tree
			Dot newPoint = new Dot(x, y);
			if (tree == null) {
				tree = new PointQuadtree(newPoint, 0, 0, width, height);
			}
			else {
			...
			// Set found to null if tree is empty, else the list returned in method findInCircle
			if (tree == null) {
				found = null;
			}
			else {
	...
	private void test2() {
		...
		tree.insert(new Dot(600,150)); // C
		tree.insert(new Dot(200,450)); // D
		tree.insert(new Dot(600,450)); // E
		int bad = 0;
		...
		bad += testFind(150,450,60,5,2,1);	// rect for all; circle for A,D; find D
		bad += testFind(750,550,10,5,2,0);	// rect for all; circle for A,E; find none
